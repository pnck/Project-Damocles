...
class CQHandler:
    ...
    def OnEvent_GroupMsg(self, subType, sendTime, fromGroup, fromQQ, fromAnonymous, msg, font):
        ...
        msg = msg.strip()
        if fromQQ == 407508177 and msg[:6] == '[SYN]':
            CQSDK.SendGroupMsg(fromGroup,'[CQ:at,qq=%d] [ACK] %s' % (fromQQ,msg[6:]))
            action = server.handle_OnEvent_GroupMsg(subType,sendTime,fromQQ,msg,font)
            if (action is not None) and (type(action) is type('action')):
                exec(action)
